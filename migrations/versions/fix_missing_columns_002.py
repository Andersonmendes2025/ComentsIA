...
def upgrade():
    ...
    missing_columns = {
        ...
        "terms_accepted": sa.Column("terms_accepted", sa.Boolean(), server_default=sa.text("0"), nullable=False),
    }

    for col_name, column in missing_columns.items():
        if col_name not in [col["name"] for col in inspector.get_columns("user_settings")]:
