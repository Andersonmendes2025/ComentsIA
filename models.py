...
class Role(db.Model):
    __tablename__ = "roles"
    id = db.Column(db.Integer, primary_key=True)
    key = db.Column(db.String(32), unique=True, nullable=False)   # admin, financeiro, diretoria, marketing_email, suporte
...
class PlanPrice(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    plan_key = db.Column(db.String(32), unique=True, nullable=False)        # free, pro, pro_anual, business, business_anual
...
class PaymentTransaction(db.Model):
    ...
    amount_cents = db.Column(db.Integer, nullable=False, default=0)         # valor realmente pago
...
class FinanceItem(db.Model):
    ...
    allocation_method = db.Column(db.String(24), default="per_user_equal")   # per_user_equal | per_revenue_share | per_transaction
...
class Coupon(db.Model):
    ...
    discount_value = db.Column(db.Integer, nullable=False)      # se percent: 10 => 10%; se fixed: centavos
    ...
    def is_valid(self) -> bool:
        ...
        if self.max_uses and (self.uses_count or 0) >= self.max_uses: return False
        return True
...
class AdminActionLog(db.Model):
    ...
    action = db.Column(db.String(64), nullable=False)              # change_role | change_perm | export | pricing_update | broadcast | etc
